"""
collision_analyzer.py (modified to use only MVCC_final.csv + GUI options list)
"""
...
# (enforces MVCC_final.csv in script directory)
# =======================
class CollisionAnalyzer:
    ...
    def __init__(self, csv_file="MVCC_final.csv"):
        ...
        # Ensure the CSV (or file named MVCC_final.csv) exists before attempting to read
...
def launch_gui():
    ...
    def load_csv_action():
        # Always load fixed MVCC_final.csv next to this script
        try:
    ...
    # New: run selected options (1-14) from a listbox (multiple selection supported)
    OPTIONS = [
        "1. Total Injured",
        "2. Total Killed",
    ...
    def run_selected_action():
        if not set_date_and_filter():
            return
        analyzer = analyzer_container.get("analyzer")
        sel = listbox_options.curselection()
        if not sel:
            messagebox.showwarning("No selection", "Select one or more options to run.")
            return

        choices = [str(i + 1) for i in sel]  # convert indices to CLI-style numbers
        results = {}
        graphs_generated = []
        outdir_for_graphs = None

        # If any graph-producing options selected, ask for output dir once
        graph_choice_indices = set(str(i + 1) for i in range(len(OPTIONS)) if str(i + 1) in {"10", "11", "12", "13", "14"})
        if any(c in graph_choice_indices for c in choices):
            outdir_for_graphs = filedialog.askdirectory(title="Select output directory for graphs") or "."
        ...
        for choice in choices:
            if choice == "0":
                ...
                results["Total Injured"] = analyzer.total_injured()
                gui_log("  Total Injured computed.")
            elif choice == "2":
                results["Total Killed"] = analyzer.total_killed()
                gui_log("  Total Killed computed.")
            elif choice == "3":
                results["Injured Categories"] = analyzer.injured_categories()
                gui_log("  Injured Categories computed.")
            elif choice == "4":
                results["Killed Categories"] = analyzer.killed_categories()
                gui_log("  Killed Categories computed.")
            elif choice == "5":
                street, cnt = analyzer.highest_accident_street()
                results["Highest Accident Street"] = f"{street} ({cnt})" if street else "N/A"
                gui_log(f"  Street With Most Accidents: {results['Highest Accident Street']}")
            elif choice == "6":
                street, cnt = analyzer.least_accident_street()
                results["Least Accident Street"] = f"{street} ({cnt})" if street else "N/A"
                gui_log(f"  Street With Least Accidents: {results['Least Accident Street']}")
            elif choice == "7":
                veh, cnt = analyzer.most_common_vehicle()
                results["Most Common Vehicle"] = f"{veh} ({cnt})" if veh else "N/A"
                gui_log(f"  Most Common Vehicle: {results['Most Common Vehicle']}")
            elif choice == "8":
                veh, cnt = analyzer.least_common_vehicle()
                results["Least Common Vehicle"] = f"{veh} ({cnt})" if veh else "N/A"
                gui_log(f"  Least Common Vehicle: {results['Least Common Vehicle']}")
            elif choice == "9":
                results["Monthly Trend"] = analyzer.accidents_by_month().to_dict()
                gui_log("  Monthly Trend computed.")
            elif choice == "10":
                gui_log("Running: Option 10 - Generate Graphs (graphs only).")
                prefix = os.path.join(outdir_for_graphs, "") if outdir_for_graphs else ""
                files = analyzer.create_graphs(out_prefix=prefix)
                graphs_generated.extend(files)
            elif choice == "11":
                gui_log("Running: Option 11 - ALL analyses.")
                results = run_all_analyses(analyzer)
                prefix = os.path.join(outdir_for_graphs, "") if outdir_for_graphs else ""
                files = analyzer.create_graphs(out_prefix=prefix)
                if files:
                    graphs_generated.extend(files)
                f = analyzer.graph_top_10_streets(out_prefix=prefix)
                if f:
                    graphs_generated.append(f)
                f = analyzer.graph_top_10_vehicles(out_prefix=prefix)
                if f:
                    graphs_generated.append(f)
                f = analyzer.graph_top_10_contributing_factors(out_prefix=prefix)
                if f:
                    graphs_generated.append(f)
            elif choice == "12":
                prefix = os.path.join(outdir_for_graphs, "") if outdir_for_graphs else ""
                ...
                gui_log(f"Unknown option: {choice}")
        ...
        if results:
            ...
            gui_log("=== End Results ===")
            # Ask to save
            save = messagebox.askyesno("Save Results", "Save analysis results to CSV?")
            if save:
                outdir = filedialog.askdirectory(title="Select output directory for report") or "."
                try:
                    path = analyzer.save_to_csv(results, out_dir=outdir)
                    gui_log(f"Report saved: {path}")
                    messagebox.showinfo("Saved", f"Report saved: {path}")
                except Exception as ex:
                    gui_log(f"Save error: {ex}")
            ...
            gui_log("Graphs generated:")
            for f in graphs_generated:
                gui_log(f"  {f}")
            messagebox.showinfo("Graphs", f"Saved {len(graphs_generated)} graphs to {outdir_for_graphs or '.'}")
        else:
            gui_log("No results or graphs generated.")
    ...
    lbl_fixed = ttk.Label(frm_top, text=".csv (fixed)")
    ...
    listbox_options = tk.Listbox(frm_options, height=8, selectmode="multiple")
    for opt in OPTIONS:
        listbox_options.insert("end", opt)
    listbox_options.pack(fill="x", expand=False)
    btn_run_selected = ttk.Button(frm_options, text="Run Selected Options", command=run_selected_action)
    btn_run_selected.pack(pady=4, anchor="e")
